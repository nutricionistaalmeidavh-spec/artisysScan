...
export function renderFleetDashboard(report: FleetReport): string {
  const cards = report.products.map((product) => {
    ...
    return `<article class="card status-${product.gate.decision.toLowerCase()}"><div class="status">${product.gate.decision}</div><h2>${escapeHtml(product.id)}</h2><p>Profile: ${escapeHtml(product.profile)}</p><p>Critical ${counts.critical} · High ${counts.high} · Medium ${counts.medium}</p>${reasons ? `<ul>${reasons}</ul>` : ''}</article>`;
  }).join('');
